...
- **永久留言板**：giscus + GitHub Discussions 保存，登录 GitHub 后可留言、编辑、删除
...
├── css/style.css       # 样式
├── css/giscus-love.css # giscus 留言板自定义主题
├── js/
│   ├── app.js          # 主逻辑（登录、照片、播放器）
│   └── hearts.js       # 爱心粒子动画
├── images/
...
留言板使用 giscus，数据保存在 GitHub Discussions。

- 留言会永久保存在仓库 Discussions 中
- 你和 Lili 都需要登录 GitHub 后才能留言
- 留言作者可以编辑或删除自己的留言
- 仓库 owner 可以在 GitHub Discussions 中管理所有留言
- 留言板主题由 `css/giscus-love.css` 控制，发布后通过 GitHub Pages 提供给 giscus iframe

仓库需要保持 Discussions 开启，并安装 giscus GitHub App。
...
- **自定义域名**：绑定专属域名
- **夜间主题**：支持切换深色模式
- **更私密的共享留言**：如果不想依赖 GitHub 登录，再选择一个带鉴权的数据服务
